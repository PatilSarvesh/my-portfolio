...
const experience = [
  {
    ...
    highlights: [
      ...
      "Engineered asynchronous Textract and Bedrock orchestration with Step Functions task tokens and SNS, eliminating polling; used strict JSON schemas for reliable extraction.",
      ...
      "Delivered features across ModelOps, TalentLens, MPC Correspondence, and MPC provider credentialing; improved template creation efficiency by 30% and cut delivery time by 20%.",
    ],
  ...
  },
  {
    ...
    location: "Bengaluru, India",
    highlights: [
      "Trained and worked as a ServiceNow developer and ITOM specialist.",
      "Built frontend validations and functionality with Glide AJAX, improving workflow efficiency by 15%.",
      "Implemented and tested pages and workflows with Automated Test Framework (ATF).",
    ],
...
const railItems = [
  { id: "top", number: "00", label: "start" },
  { id: "systems", number: "01", label: "systems" },
  { id: "experience", number: "02", label: "experience" },
  { id: "about", number: "03", label: "method" },
  { id: "now", number: "04", label: "stack" },
  ...
  useEffect(() => {
    const updateActiveSection = () => {
      ...
      railItems.forEach((item) => {
        ...
        if (section && section.getBoundingClientRect().top + window.scrollY - 32 <= marker) {
          current = item.id;
        }
      });
        ...
        <nav className="rail-nav" aria-label="Section navigation">
          {railItems.map((item) => (
            <a
              ...
              key={item.id}
            >
              <span>{item.number}</span><RailLabel>{item.label}</RailLabel>
            </a>
          ))}
        </nav>
      ...
      <div className="workbench-body">
        <header className="topbar">
          ...
          <div className="topbar-links">
            ...
            <a href="/Sarvesh_Patil_Resume.pdf" target="_blank" rel="noopener noreferrer" download="Sarvesh_Patil_Resume.pdf">PDF <ArrowIcon /></a>
          </div>
        </header>
        ...
        <main id="content">
          ...
          <section className="experience-section" id="experience" aria-labelledby="experience-title">
            <div className="content-frame">
              <div className="section-kicker"><span>02 / experience</span><span>project-led practice</span></div>
              <div className="section-intro"><h2 id="experience-title">Where I’ve<br /><i>worked.</i></h2><p>Experience across Python, AWS, GenAI, and .NET—building production systems that make complex workflows dependable.</p></div>
              <div className="experience-list">
                {experience.map((item, index) => (
                  <article className="experience-row" key={item.role}>
                    <div className="experience-period"><span>{item.period}</span><span>{String(index + 1).padStart(2, "0")}</span></div>
                    <div className="experience-main"><h3>{item.role}</h3><p className="experience-org">{item.org} <span>{item.location}</span></p><ul>{item.highlights.map((highlight) => <li key={highlight}>{highlight}</li>)}</ul></div>
                    <div className="experience-focus"><span>focus</span><strong>{item.focus}</strong></div>
                  </article>
                ))}
              </div>
            </div>
          ...
          <section className="method-section" id="about" aria-labelledby="method-title">
            <div className="content-frame method-frame">
              <div className="section-kicker"><span>03 / method</span><span>how I think</span></div>
          ...
          <section className="stack-section" id="now" aria-labelledby="stack-title">
            <div className="content-frame">
              <div className="section-kicker"><span>04 / stack</span><span>by responsibility</span></div>
          ...
          <section className="contact-section" id="contact" aria-labelledby="contact-title">
            <div className="content-frame contact-frame">
              <div className="section-kicker"><span>05 / contact</span><span>open channel</span></div>
